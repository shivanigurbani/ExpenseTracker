...
def index(request):
    ...
    paginator=Paginator(expenses,2) # Paginate 'expenses' queryset with 2 items per page
    page_number=request.GET.get('page') # Get the current page number from the request's GET parameters
    page_obj=Paginator.get_page( paginator,page_number) # Get the Page object for the current page number
    context={
        'expenses':expenses,
        'page_obj':page_obj,
    }
